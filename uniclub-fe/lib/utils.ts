...
export function calculateAverageRating(reviews: { star: number }[]): number {
...
export function getMinPrice(variants: { price: number | null; quantity: number }[], fallbackPrice: number): number {
...
export function isOutOfStock(variants: { quantity: number }[]): boolean {
